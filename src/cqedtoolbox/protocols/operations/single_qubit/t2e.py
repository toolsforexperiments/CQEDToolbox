...
class T2EOperation(ProtocolOperation):
    ...
    def _measure_dummy(self) -> Path:
        ...
        signal = (self._SIM_AMP * np.exp(-delays / self._SIM_T2E) * np.exp(2j * np.pi * self._SIM_DETUNING * delays)
    ...
    def _measure_qick(self) -> Path:
        ...
        # Store old n_echo value and set to 3 for T2E measurement
        self.old_n_echo = self.n_echos()
        logger.debug(f"Storing old n_echo value: {self.old_n_echo}, setting to {self.N_ECHOS}")
        self.n_echos(self.N_ECHOS)
    ...
    def evaluate(self) -> OperationStatus:
        """
        Evaluate if the measurement was successful and update the T2E value.
        ...
        header = (f"## T2 Echo (T2E) Measurement\n"
                  f"Measured T2 Echo time with {self.N_ECHOS} echo pulses and SNR range: {self.SNR_MIN_THRESHOLD} - {self.SNR_MAX_THRESHOLD}\n"
        ...
        if valid_components:
            ...
            logger.info(f"Updating T2E from {old_value} to {new_value}")
            self.t2e(new_value)

            self.improvements = [ParamImprovement(old_value, new_value, self.t2e)]
            ...
            msg_main = (f"### **{winner_name} Component (SELECTED)**\n"
                       ...
                       f"{self.t2e.name} shift: {old_value:.3f} -> {new_value:.3f} μs\n"
